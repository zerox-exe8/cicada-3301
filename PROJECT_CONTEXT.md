# ⚡ Kyro Discord Bot — Comprehensive Technical Specification & AI System Context

> **Audience**: AI Coding Assistants (Claude / GPT), Lead System Architects, and Software Engineers.  
> **Purpose**: This document provides the complete, authoritative, and up-to-date architectural blueprint, database schemas, code conventions, and feature catalog of the **Kyro** codebase.
...
- If no reminder was sent in the current billing cycle, sends a `KyroContainer` DM notice to the server owner prompting them to renew via `?buy`.
...
└── src/
    ...
    │
    ├── utils/
    │   ├── containers.py        # KyroContainer (Type 17), send/edit container response
...
2. **Components V2 Only**: Build user-facing cards with `KyroContainer` (`src.utils.containers`). Do not use old `discord.Embed`.
